...
contract StudentFinance{
    ...
    function GetAddressFinancier() private view returns (address)
    {
        return financier;
    }
    ...
    function GetDate() public view returns (uint y, uint m, uint d)
    {
        uint date = 0;
        ...
        {
            date = studentrecords[msg.sender].startdate;
            require(date != 0);
        }
        return (getYear(date), getMonth(date), getDay(date));
    }
    
    function GetDateContract() public view returns (uint y, uint m, uint d)
    {
        return (getYear(contractdate), getMonth(contractdate), getDay(contractdate));
    }
    ...
    function GetInformation() public view returns (uint i, uint ip, bool s)
    {
        RequireIsStudent;
        return(studentrecords[msg.sender].income, studentrecords[msg.sender].incomeparents, studentrecords[msg.sender].studying);
    }
    ...
    function GetNumberActiveStudents() public view returns (uint)
    {
        return activestudents.length;
    }
    
    function InitializeStudentRecord(uint income, uint incomeparents) public
    {
        RequireIsStudent();
        studentrecords[msg.sender] = Student(block.timestamp, income, incomeparents, true, new bytes32[](0), "", 0, 0);
    }
    ...
    function GetToPayThisMonth() public view returns (uint)
    {
        RequireIsStudent();
        return studentrecords[msg.sender].topaythismonth;
    }
    
    function GetRepaymentYearsLeft() public view returns (uint)
    {
        RequireIsStudent();
        bytes32 empty = "";
        if(studentrecords[msg.sender].repaymenttype == empty)
        {
            return 0;
        }
        uint y;
        uint p;
        (y, p) = GetRepaymentType(studentrecords[msg.sender].repaymenttype);
        if(studentrecords[msg.sender].repaymentstartdate == 0)
        {
            return y;
        }
        else
        {
            uint monthsdifference = GetDifferenceInMonths(studentrecords[msg.sender].repaymentstartdate, block.timestamp);
            return monthsdifference >= (y * 12) ? 0 : (y * 12) - monthsdifference;
        }
    }
    
    function GetOwnRepaymentType() public view returns (bytes32)
    {
        RequireIsStudent();
        require(IsActiveStudent());
        return studentrecords[msg.sender].repaymenttype;
    }
    
    function GetNumberOfLoans() public view returns (uint)
    {
        RequireIsStudent();
        return studentrecords[msg.sender].loans.length;
    }
    
    function GetLoanLog(uint index) public view returns (bytes32 l, uint c, uint y, uint m, uint d)
    {
        RequireIsStudent();
        require(studentrecords[msg.sender].loans.length > index);
        bytes32 loan = studentrecords[msg.sender].loans[index];
        uint expirydate = studentrecords[msg.sender].loanlogs[loan].expirydate;
        return (loan, studentrecords[msg.sender].loanlogs[loan].monthlycreditleft, getYear(expirydate), getMonth(expirydate), getDay(expirydate));
    }
    
    function GetMaxAmount(bytes32 loan) public view returns (uint)
    {
        return loantypes[loan].maxamount;
    }
